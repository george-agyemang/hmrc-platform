...
const dbQuery = async (env, query, params = []) => {
...
// ── Health ────────────────────────────────────────────────────
router.get('/health', (req) => json({ status: 'ok', environment: 'production' }, 200, req));

// ── Auth: Initiate HMRC OAuth ─────────────────────────────────
router.get('/auth/hmrc', (req, env) => {
  ...
  const authUrl = `${env.HMRC_AUTH_URL}?${params}`;
...
router.post('/users/register', async (req, env) => {
  ...
  const res = await fetch(`${env.SUPABASE_URL}/rest/v1/User`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'apikey': env.SUPABASE_ANON_KEY,
    ...
    body: JSON.stringify({ id, email, name, passwordHash: hash }),
  });

  if (!res.ok) {
    const e = await res.json();
    if (e[0]?.code === '23505') return err('Email already exists', 400, req);
...
router.post('/users/login', async (req, env) => {
  ...
  const res = await fetch(`${env.SUPABASE_URL}/rest/v1/User?email=eq.${encodeURIComponent(email)}&select=*`, {
    headers: {
      'apikey': env.SUPABASE_ANON_KEY,
      'Authorization': `Bearer ${env.SUPABASE_ANON_KEY}`,
    },
  ...
  if (!valid) return err('Invalid email or password', 401, req);
...
router.get('/users/me', async (req, env) => {
  ...
  const res = await fetch(`${env.SUPABASE_URL}/rest/v1/User?id=eq.${session.userId}&select=id,email,name,createdAt`, {
...
router.get('/businesses', async (req, env) => {
  ...
  const res = await fetch(`${env.SUPABASE_URL}/rest/v1/Business?userId=eq.${session.userId}&select=*`, {
    headers: { 'apikey': env.SUPABASE_ANON_KEY, 'Authorization': `Bearer ${env.SUPABASE_ANON_KEY}` },
  });
...
router.post('/businesses', async (req, env) => {
  ...
  const res = await fetch(`${env.SUPABASE_URL}/rest/v1/Business`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'apikey': env.SUPABASE_ANON_KEY,
    ...
    body: JSON.stringify({ id: crypto.randomUUID(), userId: session.userId, businessName, businessType, status: status || 'ACTIVE', vrn: vrn || null, utr: utr || null, crn: crn || null }),
  });
...
router.get('/businesses/:id', async (req, env) => {
  ...
  const res = await fetch(`${env.SUPABASE_URL}/rest/v1/Business?id=eq.${req.params.id}&userId=eq.${session.userId}&select=*`, {
    headers: { 'apikey': env.SUPABASE_ANON_KEY, 'Authorization': `Bearer ${env.SUPABASE_ANON_KEY}` },
  });
...
// ── Submissions: VAT Obligations ──────────────────────────────
router.get('/submissions/obligations/vat/:businessId', async (req, env) => {
  const session = await getSession(req, env);
  if (!session) return err('Not authenticated', 401, req);

  const bizRes = await fetch(`${env.SUPABASE_URL}/rest/v1/Business?id=eq.${req.params.businessId}&userId=eq.${session.userId}&select=vrn`, {
    headers: { 'apikey': env.SUPABASE_ANON_KEY, 'Authorization': `Bearer ${env.SUPABASE_ANON_KEY}` },
  });
  const biz = (await bizRes.json())[0];
  if (!biz?.vrn) return err('Business not found or no VRN', 404, req);

  // Get tokens from session (stored in cookie)
  const tokenRes = await fetch(`${env.SUPABASE_URL}/rest/v1/HmrcToken?userId=eq.${session.userId}&select=accessToken`, {
    headers: { 'apikey': env.SUPABASE_ANON_KEY, 'Authorization': `Bearer ${env.SUPABASE_ANON_KEY}` },
  });
  const tokens = await tokenRes.json();
...
router.post('/submissions/vat/nil', async (req, env) => {
  ...
  const bizRes = await fetch(`${env.SUPABASE_URL}/rest/v1/Business?id=eq.${businessId}&userId=eq.${session.userId}&select=vrn`, {
    headers: { 'apikey': env.SUPABASE_ANON_KEY, 'Authorization': `Bearer ${env.SUPABASE_ANON_KEY}` },
  });
  const biz = (await bizRes.json())[0];
  if (!biz?.vrn) return err('Business not found or no VRN', 404, req);
  ...
  const payload = {
    periodKey, vatDueSales: 0, vatDueAcquisitions: 0, totalVatDue: 0,
    vatReclaimedCurrPeriod: 0, netVatDue: 0, totalValueSalesExVAT: 0,
    totalValuePurchasesExVAT: 0, totalValueGoodsSuppliedExVAT: 0,
  ...
  const sub = { id: crypto.randomUUID(), businessId, taxType: 'VAT', submissionType: 'NIL', periodKey, periodStart, periodEnd, status: hmrcRes.ok ? 'ACCEPTED' : 'REJECTED', hmrcReceiptId: hmrcData.formBundleNumber || null, payload, hmrcResponse: hmrcData, submittedAt: new Date().toISOString() };

  await fetch(`${env.SUPABASE_URL}/rest/v1/Submission`, {
    method: 'POST',
...
router.get('/submissions/:businessId', async (req, env) => {
  ...
  const res = await fetch(`${env.SUPABASE_URL}/rest/v1/Submission?businessId=eq.${req.params.businessId}&order=createdAt.desc`, {
    headers: { 'apikey': env.SUPABASE_ANON_KEY, 'Authorization': `Bearer ${env.SUPABASE_ANON_KEY}` },
  });
